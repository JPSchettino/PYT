...
from layout import sens_layout
import gspread
...
class DataManager:
    ...
    def update_data(self, selected_base):
        if selected_base == 'Wafer':
            self.sales = pd.read_csv('Biscoito_doce_Recheado.csv')
        elif selected_base == 'Pipoca':
            self.sales = pd.read_csv('Pipoca.csv')
...
# Adicione o parâmetro `external_stylesheets` na criação do aplicativo Dash
def create_app(server, url_base_pathname):
    ...
    app.layout = sens_layout(faixa_etaria_options, se_declara_options, classe_eco_servicos_options,
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        ...
        return create_bar_chart(filtered_df, 'Distribuição por Faixa Etária', 'Faixa etária:')
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        return create_pie_chart2(filtered_df, 'Distribuição por Gênero', 'Você se declara:')
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        return create_bar_chart(filtered_df, 'Distribuição por Profissão', 'Você trabalha em algum dos seguimentos listados abaixo?')
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        return create_pie_chart(filtered_df, 'Distribuição por Classe Econômica', 'Classe Econômica com Serviços')
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        return create_horizontal_bar_chart(filtered_df, 'Distribuição por Escolaridade', 'Qual a sua escolaridade?',left = 180)
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        return create_pie_chart(filtered_df, 'Distribuição por "Você tem filhos"', 'Você tem filhos?')
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada,y_column = 'Quais das opções abaixo se aplicam a você?'):
        ...
        stacked_df.info(10)
        return create_bar_chart(stacked_df, 'Aptidão para a avaliação', y_column)
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        return create_pie_chart(filtered_df, 'Distribuição por Renda', 'Qual das oções a seguir melhor descreve a sua rende mensal familiar?')
    ...
    def update_graph(faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada,y_column = 'Temos abaixo uma lista de diferentes alimentos para o consumo. Qual deles você adquire / consome geralmente? (Por favor, indique todas as opções que se aplicarem)'):
        sales = data_manager.get_data()
        filtered_df = filter_data(sales, faixa_etaria, segment, classe_eco_servicos, reside_sp, tem_filhos, resposta_selecionada)
        stacked_df = stack_dataframe(filtered_df, y_column)
        stacked_df.info(10)
